...
<script>
    ...
    export default {
        ...
        methods: {
            initMap () {
                this.map = L.map(this.$zircle.getCurrentViewName(), {attributionControl: false, zoomControl: false}).setView([41.01, 28.87], 14)
                L.tileLayer(
                ...
                this.markers.push(
                    L.marker([41.026050, 28.878487]).bindPopup("Hoop Evdeyim, Saniyede !").addTo(this.map).openPopup(),
                    L.marker([41.025973, 28.889328]).bindPopup("Hoop İşteyim, Saniyede !").addTo(this.map).openPopup(),
                )
                this.active = 1
                this.showMe(1, 'green')
            },
